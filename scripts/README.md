...
`..\manifests\managed-applications.json` is the canonical managed-package definition: the compiled product loads it, and `Export-AVWorkstationToolkitBaselineManifest.ps1` generates the winget baseline from it. The retired `AppProfiles.psd1` fixture is still read by these legacy operator scripts and by the characterization suite; source QA validates it against the canonical JSON, so it cannot drift. Author every managed-package change in the JSON. Do not add corporate security, device-management, VPN, or remote-support software to either representation.
