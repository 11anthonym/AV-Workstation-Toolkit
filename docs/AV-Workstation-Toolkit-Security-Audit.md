...
- Package QA passed 16 of 16 checks covering a lone EXE copied into an empty download directory, deterministic no-rewrite runtime extraction and cache repair, EXE/MSI identity and byte parity, CycloneDX SBOM and schema-v3 release metadata, strict vendor-bridge request rejection, the packaged vendor-bridge self-test, checksums, one-file ZIP parity, WPF control/workflow smoke, diagnostics, and non-registering MSI extraction.
