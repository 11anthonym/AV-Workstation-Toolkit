...
**Scope:** standalone launcher, embedded-runtime extraction and verification, MSI/portable/offline-bundle packaging, WPF/XAML frontend, WinGet, operational external, and commercial AV awareness catalogs, normalized metadata and filtering, vendor release awareness, HTTPS/SFTP/Credential Manager bridge, vendor cache, core module, action worker, terminal deployment/maintenance entry points, readiness and snapshot scripts, tests, logging, release artifacts, publication privacy, third-party notices, and release provenance
**Execution context reviewed:** Windows PowerShell 5.1, WPF, Desktop App Installer/winget 1.29.280, standard-user token

## Executive result

No open critical or high-severity code finding remains in the v1.1 implementation. The same independently runnable executable is delivered directly, through the per-machine MSI, and in the one-file ZIP. It restores and verifies its embedded runtime beneath the current user's LocalAppData directory before loading PowerShell and still refuses elevated startup. Unsigned artifacts do not provide publisher authentication.

This pass also separated broad commercial AV knowledge from operational providers. The combined catalog now has 332 records, but only the original exact-ID WinGet allowlist can reach a change request. All 303 external records remain on manual holds; 278 of them are the commercial awareness layer. Schema 3 validates licensing, access, lifecycle, impact, platform, official source, and parent relationships without interpreting catalog knowledge as deployment approval.
...
|---|---|---|
| High | A packaged launcher could become an elevation bridge if it accepted arbitrary scripts/arguments or ran with an administrator token. | Resolved. `AVWorkstationToolkit.exe` is `asInvoker`, refuses elevation, pins inbox Windows PowerShell and the single frontend script, and exposes only bounded smoke/diagnostic switches. |
...
| Medium | A partial, stale, or modified package could load mismatched XAML and scripts. | Resolved for accidental/cross-version drift. The executable carries the audited runtime as compile-time resources, rejects invalid or duplicate embedded paths and reparse-point cache paths, restores changed cache files, and verifies extracted SHA-256 hashes before launch. Trusted publisher authentication still requires signing. |
...
| High | Repository scripts and XAML could previously be loaded by an elevated PowerShell process from a user-writable directory. | Resolved. Every executable entry point checks the token before importing a local module or loading XAML and refuses elevation. Installers may request UAC independently. |
...
| Medium | Deployment and maintenance scripts duplicated execution/policy logic and could drift from the desktop worker. | Resolved. Both terminal paths now construct the same strict request and delegate to `Invoke-AVWorkstationToolkitAction.ps1`. |
...
| Low | The launcher used process-level `ExecutionPolicy Bypass`. | Resolved. It now uses `RemoteSigned`; Group Policy still takes precedence. |
| Low | Catalog refresh relied on implicit discovery of `Import-PowerShellDataFile`, causing availability failures in a long-lived host with missing Utility-module command discovery. | Resolved. The core explicitly imports `Microsoft.PowerShell.Utility` and module-qualifies the parser; a fresh-process regression disables implicit module autoloading. |
...
| Low | Runtime and inventory troubleshooting required inspection of PowerShell output and local files. | Resolved. The desktop exposes read-only sanitized diagnostics with per-source state, subsystem counts, copy, and export; it reads no credential values and grants no action authority. |
...
| Low | Three entry points duplicated request schema creation and worker launch behavior. | Resolved. The versioned core module now owns both operations; static and behavioral tests enforce use by GUI and terminal callers. |
...
| High | Vendor credentials could leak through process arguments, environment variables, logs, plans, or source-only PowerShell transport. | Resolved. The compiled bridge accepts bounded JSON only over redirected standard input. Password fields are cleared after use, operational errors are sanitized, and saving uses a Windows generic credential scoped to AV Workstation Toolkit, host, port, and username only after successful authentication. |
...
| Medium | The compiled vendor bridge accepted unknown JSON properties, allowing request-schema drift to pass silently. | Resolved. Source-generated deserialization now rejects unmapped members, and packaged-artifact QA submits and confirms rejection of a hostile extra property. |
...
| Medium | Release provenance did not hash its own release manifest, and an existing tagged GitHub release could be overwritten in place. | Resolved. The manifest is generated before the checksum list, the checksums cover the EXE, MSI, ZIP, third-party notice, SBOM, and manifest, and the tagged workflow refuses an existing release instead of uploading with replacement semantics. |
...
| Medium | Publication material could omit runtime notices, overstate privacy, or imply that catalogued vendor products were redistributed. | Mitigated. The release publishes and embeds reviewed project/.NET notices, the privacy policy describes bounded automatic network checks, and the normal distribution boundary explicitly excludes vendor software and workstation evidence. A root open-source license remains a required owner decision before publication. |

## Security controls verified

- 29-package exact-ID WinGet allowlist, 25 operational external records, and 278 awareness records, with cross-catalog duplicate detection and explicit VirtualBox exclusion;
...
- the fresh source tree passed the same high-confidence token/private-key scan
  before its proposed zero-parent root commit was staged;
...
- the release checksum list covers the distributables, versioned third-party notice, SBOM, and release manifest while excluding only itself to avoid a circular digest;
...
7. **Release provenance.** The fresh repository starts from an owner-reviewed, zero-parent root commit with locked dependencies and a publication-audited source tree. Keep any future GitHub repository private until the owner completes the artifact, privacy, security, and release review, and use branch protection or required review when the hosting plan makes it available. Action SHAs and dependency locks must be deliberately reviewed when updated.
...
9. **Update metadata.** WinGet does not currently expose JSON for the `list --upgrade-available` command used by this Windows PowerShell 5.1 application. Installed/missing decisions are structured, while update discovery retains guarded console parsing. Any empty, failed, or truncated update inventory makes the entire plan unselectable, so this residual can suppress an update but cannot authorize a duplicate install.
...
14. **GitHub-native scanning.** This fresh local repository has no remote, so GitHub-hosted dependency, code, or secret-scanning status cannot yet be assessed. Local dependency, source secret, analyzer, compiler, package, and Defender checks remain required; enabling server-side features is a later repository-administration decision and may depend on the hosting plan.
...
- 106 of 106 full automated safety and regression checks passed; the CI-safe subset passed 100 of 100.
- Package QA passed 14 of 14 checks covering a lone EXE copied into an empty download directory, deterministic no-rewrite runtime extraction and cache repair, EXE/MSI identity, CycloneDX SBOM and schema-v3 release metadata, strict vendor-bridge request rejection, the packaged vendor-bridge self-test, checksums, one-file ZIP parity, WPF control smoke, and non-registering MSI extraction.
...
- Deterministic provider tests cover all 332 records, every normalized discipline, licensing/access composition, 278 commercial awareness holds, representative registry entries, direct-download lookalike/stale URLs, signed-cache tampering, and SFTP XML DTD/traversal/incomplete feeds.
...
- Crestron regression tests resolve each of the seven child versions through the one parent feed and verify inherited host, root, size, product, and publisher policy.
- Microsoft Defender, using signature 1.457.275.0 updated on 2026-08-21, completed a custom scan of the rebuilt release directory with no threat detection.
- Automated layout inspection loads 45 named controls and all 332 records, verifies the telemetry-free header, conventional menu, intentional sidebar scrolling, six domain-aware sortable columns, composed quick views, wrapped purpose/restriction access, and narrow-window horizontal access, constructs the read-only details, Safety & Security, About, and Diagnostics dialogs, and exercises the packaged WPF workflow at 1040x760, 1280x860, 1440x900, and 1920x1080. Geometry and UI-state behavior passed; the current automated frames were classified unavailable, so interactive visual review remains a production-release gate.
- PowerShell 5.1 parser validation passed for every script and module.
