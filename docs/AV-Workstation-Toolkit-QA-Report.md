...
Technical QA does not authorize publication. AV Workstation Toolkit is licensed under [Apache-2.0](../LICENSE), but this fresh repository remains local-only and no public release or SignPath submission was made during this pass. The exported source tree has no inherited Git history, tags, or remote; owner review and creation of the fresh root commit remain publication gates.
...
| Catalog reproducibility | Pass; 113 authoritative vendor files compile to the tracked 278-record runtime artifact, and release preflight rejects source/artifact drift before clearing prior output |

## Automated coverage

| Area | Result | Evidence |
|---|---|---|
| Catalog schema, policies, unique IDs, profiles, risks, and forbidden products | Pass | 29 managed WinGet, 25 operational external, and 278 awareness records aggregate to 332 unique records |
...
| Schema 3 commercial AV metadata and queries | Pass | Validated disciplines, role/priority, lifecycle, licensing/access separation, account gates, impact, platform, official URI, explicit unknowns, and composed query filters across 303 external records |
...
| Publication privacy and history | Pass for staged source | The fresh source tree passed identifier/path review and Gitleaks 8.30.1; the repository has no inherited commits, tags, refs, remote, archive-object alternate, or staged private operational record |
...
| Final artifact malware scan | Pass | Microsoft Defender signature 1.457.323.0 (updated 2026-08-24); no detection in the rebuilt release directory |

## Visual QA

Current automated WPF checks verify 45 named controls, all 332 catalog records, catalog/manufacturer/discipline composition, the File/View/Tools/Help menu and shared command wiring, the cleaned telemetry-free header, dark About identity, six domain-aware sortable columns and visible direction, numeric version ordering, row/checkbox selection retention, Missing/Updates/All quick views, filter/sort/refresh persistence, intentional sidebar scrolling, closed ComboBox selected-value visibility, narrow-window horizontal access, compact purpose/restriction wrapping with full-text access, the read-only detail/safety/diagnostics surfaces, and the packaged workflow smoke path. Deterministic layout metrics pass at 1040x760, 1280x860, 1440x900, and 1920x1080 with Missing, Updates, and All states represented.

The deterministic capture path classified all four current frames as unavailable rather than treating black or foreign frames as visual evidence. Geometry and UI-state behavior passed, but the cleaned header, sort indicators, active quick views, About dialog, open menu popups, hover/focus/disabled states, pending-reboot banner, and tooltip behavior still require a trustworthy interactive desktop review before production publication.
