...
The v1.1 source and package totals below are refreshed by the final Phase 6 validation run. The non-shipping C# solution now includes a conventional compiled WPF App using the typed Domain, read-only planning coordinator, Phase 3 Windows providers, typed sanitized diagnostics, read-only catalog/provider details, and a strict in-memory action-request boundary. Deterministic tests and PowerShell-to-C# parity cover filters, sorting, checkboxes, counts, warnings, asynchronous refresh, stale-result suppression, detail/provenance semantics, diagnostics/source quality, request creation/validation/authorization, and explicit mutation refusal. The launcher, PowerShell-hosted shipping WPF application, shipping request persistence, shipping worker, vendor bridge, MSI, ZIP, and normal execution path remain unchanged.
...
| C# migration architecture and semantic parity | Pass, non-shipping | Locked .NET 10 solution, 59 MSTest cases, 19 managed planning results, 36 focused Domain cases, 37 read-only provider cases, 4 presentation cases, 4 read-only details/diagnostics cases, and 26 strict action-request cases compare with shipping PowerShell semantics |
