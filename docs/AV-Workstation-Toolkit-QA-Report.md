...
The v1.1 source and package totals below remain the final shipping validation baseline from Phase 6; Phase 7 changed only non-shipping migration code, tests, endpoint-trust policy, and documentation, so it used the proportional migration validation described below rather than rebuilding release packages. The non-shipping C# solution now includes a conventional compiled WPF App using the typed Domain, read-only planning coordinator, Phase 3 Windows providers, typed sanitized diagnostics, read-only catalog/provider details, strict action requests, and an uncomposed IPC/file-lifecycle implementation. Deterministic tests and PowerShell-to-C# parity cover filters, sorting, checkboxes, counts, warnings, asynchronous refresh, stale-result suppression, detail/provenance semantics, diagnostics/source quality, request creation/validation/authorization, contained test-root persistence, progress/results, cancellation/lifecycle state, and explicit mutation refusal. The launcher, PowerShell-hosted shipping WPF application, production request persistence, shipping worker, vendor bridge, MSI, ZIP, and normal execution path remain unchanged.
...
| C# migration architecture and semantic parity | Pass, non-shipping | Locked .NET 10 solution, 72 MSTest cases, 19 managed planning results, 36 focused Domain cases, 37 read-only provider cases, 4 presentation cases, 4 read-only details/diagnostics cases, 26 strict action-request cases, and 27 IPC/file-lifecycle cases compare with shipping PowerShell semantics |
...
| Compiled C# WPF presentation | Pass, non-shipping | `x:Class` App/MainWindow plus details/diagnostics windows, MVVM bindings, deterministic compiled-process smoke, composed filters/sorting, selection retention, warning presentation, sanitized diagnostics, provenance detail groups, validated non-executing official-link intents, and hard read-only action refusal |
...
| Embedded launcher payload and constrained PowerShell arguments | Pass | Static review plus isolated-download diagnostics and cache-repair test |
| Endpoint-trust process and packaging policy | Pass | Seven reviewed direct-launch categories plus static rejection of encoded/bypass, security-tampering, proxy-binary, generic-shell, temporary-script, and packer patterns |
...
| Compiled action-request boundary | Pass, non-shipping | Typed schema-only model; strict types/unknown/duplicate/schema/null/ID/size rejection; direct-child/reparse path policy; exact plan-action/authority/risk checks; duplicate IDs authorized once; no live persistence or worker launch |
| Compiled IPC/file lifecycle | Pass, non-shipping | Explicit-root create-new/flush/atomic-move persistence tested only in temporary roots; canonical artifact derivation; bounded incremental progress and strict result correlation; cooperative cancellation and terminal-state model; compiled App has no store or worker composition |
...
| Current rebuilt artifact malware scan | Pass | Development EXE SHA-256 `A95321BE3C193C1CC67B1BC0C635CEDAD07D9DB9C3E61ADB80042589FDF5E7C5`, Defender platform 4.18.26070.9, engine 1.1.26070.7, signature 1.457.350.0 (updated 2026-08-26), custom release-directory scan reported no detection |
...
The packaged WPF smoke path is automated and passed. It deliberately does not enter credentials, save trust, download vendor software, or launch an installer.
