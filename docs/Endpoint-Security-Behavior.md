...
Unsigned development builds are identified as such. A production-channel build requires an externally provisioned code-signing certificate, signs and RFC3161-timestamps the EXE and MSI, verifies both signatures, and records signer/timestamp state. Current artifacts are unsigned and the existing certificate-store/PFX capability is not a SignPath integration or approval. Future hosted signing must preserve the signed EXE inside the signed MSI and require deliberate approval as described in the [code signing policy](Code-Signing-Policy.md).
