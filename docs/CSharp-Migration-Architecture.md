...
### Compiled action-request boundary (non-shipping)
...
The compiled Application model serializes only this schema, applies strict JSON types, rejects unknown/duplicate/missing properties, validates IDs, and reuses Domain selection/reboot/risk policy against exact managed plan actions. The Infrastructure file policy is read-only and enforces an absolute non-root data path, `logs\requests` direct-child containment, filename/schema ID agreement, bounded reads, and reparse rejection. Neither service is composed into the compiled App. No live request writer, worker launcher, result watcher, cancellation channel, WinGet mutation, or execution implementation exists in the migration runtime.
...
- **Migration implementation present but not active:** typed C# deterministic domain logic, read-only Windows providers, sanitized diagnostics, catalog/provider detail projections, strict in-memory action-request creation/validation, a compiled WPF presentation, and dual-engine tests compile into non-shipping projects. The compiled App can read the source checkout catalogs, collect read-only workstation facts, display a plan, show read-only details, and present sanitized diagnostics. It does not compose the request factory/path reader, cannot persist a live request, has no action-worker or vendor-transport dependency, and explicitly refuses install/update requests. No launcher or package references these assemblies.
