...
- **Migration implementation present but not active:** typed C# deterministic domain logic, read-only Windows providers, sanitized diagnostics, catalog/provider detail projections, strict action-request and IPC contracts, a compiled WPF presentation, and a separate fake-executor-only worker process harness compile into non-shipping projects. The worker harness reauthorizes every package and writes correlated progress/results only beneath an injected test root. The compiled App still does not compose the request factory, path policy, protocol store, or worker; cannot persist a live request; has no action-worker or vendor-transport dependency; and explicitly refuses install/update requests. No launcher or package references the worker or migration App.
