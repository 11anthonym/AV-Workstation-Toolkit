...
The compiled application and supported PowerShell workflows install and maintain only approved user applications in `manifests/managed-applications.json`, the canonical managed-package authoring source. `scripts/AppProfiles.psd1` is a retired characterization fixture and is not an operational authority. The external and commercial AV manifests add inventory, provider, access, and role awareness; they do not expand the automated change boundary.
...
1. Add one entry to the canonical `manifests/managed-applications.json` with profile, exact winget ID, risk class, and validation note. Do not add it to the retired `scripts/AppProfiles.psd1` characterization fixture.
