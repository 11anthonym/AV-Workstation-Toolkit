...
Use search, profile, catalog-policy, manufacturer, and discipline filters to narrow the table. Manufacturer choices are generated from the loaded catalog and compose with every other filter. Catalog presets cover P1/onsite candidates, free/public tools, dealer access, licensing, drivers, services/listeners, firmware utilities, lifecycle, unmanaged records, and installed software with a limited source. Disciplines are metadata-based and separate from manufacturer ecosystems. Select a row and choose `View details`, or double-click it, to inspect its identity, installed state, access restrictions, compatibility fields, system impact, evidence, and official links. This view is read-only and does not grant installation authority. Purpose and restriction text uses a compact two-line row preview; hover it or open details for the complete value. `Diagnostics` shows sanitized runtime, WinGet, reboot, catalog, and per-registry-source health. Warning cards name the failed check, explain the impact, and provide a safe next action above the complete technical snapshot; diagnostics can still be copied or exported without reading credentials.

When a workstation check fails, the main banner identifies the affected subsystem instead of showing only a warning count. `View details` opens the diagnostic explanation and `Check again` reruns the read-only plan. For example, if installed WinGet inventory succeeds but its update query is malformed or unavailable, installed versions remain visible but affected managed applications show `Check unavailable` rather than `Current`; no update action is authorized until the update check succeeds. A catalog metadata-review warning is descriptive and does not itself grant or remove package authority.
...
The conventional menu keeps less-frequent commands out of the workstation action area. `File` contains plan export, logs, and exit; `View` contains refresh and Diagnostics; `Tools` checks system state; and `Help` contains Device catalog updates, Managed app catalog updates, the read-only Safety & Security explanation, and About. About identifies the AV Workstation Toolkit product version and whether the app is packaged or running from source. WinGet, launcher/runtime, privilege, and inventory health remain in Diagnostics instead of occupying the normal header. The refresh and Diagnostics buttons remain available as common shortcuts. If the left filter area exceeds the window height, its own vertical scrollbar provides access to every filter, quick view, selection command, and plan count.

### Managed app catalog updates

Open **Help > Managed app catalog updates...** to see the effective signed revision, its source, update availability, verification result, and restart requirement. Choose **Check now**, review the reported revision, then choose **Download update** only when the application reports a verified update. The newly activated revision is not used by the running process; restart AV Workstation Toolkit, reopen this window to confirm the revision, and refresh the package list. The compiled worker independently verifies the stored catalog and rejects an action when its revision differs from the application's request. Offline, rejected, tampered, incompatible, or same/older downloads leave the current signed catalog usable.

This control is unavailable as a production update path until the owner has provisioned the distinct managed public key, published the signed channel, and shipped a signed embedded baseline. Source-checkout runs continue to use canonical `manifests/managed-applications.json` as development revision 0.
