...
- **Wave 5 complete — assistive listening / network audio.** Added five exact identities across Listen Technologies, Williams AV, and Audinate. ListenWIFI Manager is limited to the LA-490 workflow, legacy LW-100P and Williams transmitters retain browser/manual workflows as unresolved desktop coverage, and Dante Controller is related only to the exact Dante AVIO USB adapter—not inferred across Dante hardware.
- **Wave 6 complete — power / control / utility hardware.** Added six exact identities for Crestron CEN-IO, WattBox, SurgeX, and Middle Atlantic. Toolbox is limited to documented CEN-IO discovery before embedded-web setup; power products remain known hardware with unresolved desktop coverage, so cloud/browser/control-system association creates no software or execution authority.
