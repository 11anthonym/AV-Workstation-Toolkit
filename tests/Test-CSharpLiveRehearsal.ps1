...
if ($isElevated) {
    Write-Output 'CSHARP_LIVE_REHEARSAL_SKIPPED reason=elevated-host'
    return
}
if (-not $NoBuild) {
    & dotnet build (Join-Path $repositoryRoot 'AVWorkstationToolkit.slnx') -c Release --no-restore --nologo
    if ($LASTEXITCODE -ne 0) { throw 'C# migration build failed before live rehearsal.' }
}
...
Write-Output ("CSHARP_LIVE_REHEARSAL_OK package={0} action={1} installed={2} available={3} progress={4} worker={5} mutation=no" -f
