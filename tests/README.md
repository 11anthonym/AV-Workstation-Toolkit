...
Worker boundary tests use the non-shipping development host or intentionally malformed requests; none of them installs, updates, or removes software. PowerShell action-worker tests use only out-of-scope or intentionally malformed requests, which the worker rejects before reading workstation inventory or calling winget.
